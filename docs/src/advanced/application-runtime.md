...
`caelix-actix` provides `Application`.
...
Controller route wrappers are generated by `#[controller]`. They collect request headers into `RequestContext`, run guards, run interceptors, call the controller method, convert the result to `caelix_core::HttpResponse`, then convert it to `actix_web::HttpResponse`.
