...
Generated applications depend on `caelix = "0.0.25"` from crates.io. The generator refuses to overwrite existing files, so it is safe to run against a feature name and stop when a generated file already exists.
