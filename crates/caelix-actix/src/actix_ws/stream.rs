...
impl Stream for MessageStream {
    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        loop {
            validate_raw_close_reason(&this.buf)?;
            let Some(frame) = this.codec.decode(&mut this.buf)? else {
                break;
            };
...
/// Validates close-reason bytes before actix-http's lossy UTF-8 conversion.
fn validate_raw_close_reason(buf: &BytesMut) -> Result<(), ProtocolError> {
    if buf.len() < 2 || buf[0] & 0x0f != 0x08 {
        return Ok(());
    }
    ...
    let payload_len = match buf[1] & 0x7f {
        ...
        127 => {
            ...
            header_len = 10;
            let length = u64::from_be_bytes(buf[2..10].try_into().expect("length checked"));
            usize::try_from(length).map_err(|_| ProtocolError::Overflow)?
        }
        _ => unreachable!(),
    };
    ...
    let mask = if masked {
        ...
        let mask: [u8; 4] = buf[header_len..header_len + 4]
            .try_into()
    ...
    if buf.len() < header_len.saturating_add(payload_len) {
        return Ok(());
    }
    if payload_len == 1 {
    ...
    let reason = payload[2..]
        .iter()
        .enumerate()
        .map(|(index, byte)| mask.map_or(*byte, |mask| byte ^ mask[(index + 2) % 4]))
        .collect::<Vec<_>>();
    std::str::from_utf8(&reason)
        .map_err(|error| ProtocolError::Io(io::Error::new(io::ErrorKind::InvalidData, error)))?;
    Ok(())
}
